...
This directory is the community registry for `agentically`. Each subdirectory is an **agent system** that developers can discover and install with the `agentically` CLI.

## Directory Structure

```
agents/
  my-agent-system/           ← each folder is one agent system
    ...
    README.md                ← required: describes what the agent system does
```
...
Agent systems that don't use `prompts/` or `skills/` directories still work.
...
## Adding Your Agent System

1. Fork this repository.
2. Create a new folder under `agents/` with a kebab-case name (e.g., `agents/my-agent-system/`).
...
- Use kebab-case: `my-great-agent`
- Be descriptive but concise
- Avoid generic names like `agent` or `system`
