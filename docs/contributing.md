...
- An example usage snippet with `agentically create <name>`

### 4. Keep it platform-agnostic

Agent systems in this registry should work regardless of the user's coding platform (Copilot, Claude, Kiro, etc.). The `agentically` CLI handles platform-specific adaptations via `--platform`. If your agent system is inherently platform-specific, document this clearly in your `README.md`.
...
Once merged, your agent system is immediately available via:
...
Open an issue on the [agentically GitHub repository](https://github.com/ai-superpowers/agentically).
